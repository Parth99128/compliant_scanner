...
const Table = React.forwardRef<HTMLTableElement, React.HTMLAttributes<HTMLTableElement>>(
  ({ className, ...props }, ref) => (
    <div className="w-full overflow-x-auto rounded-md border border-slate-200">
      <table ref={ref} className={cn("w-full caption-bottom bg-white text-sm", className)} {...props} />
    </div>
  )
);
...
const TableHead = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn(
        "bg-slate-50 px-3 py-2.5 text-left align-middle text-[11px] font-bold uppercase tracking-wider text-slate-600",
        className
      )}
      {...props}
    />
